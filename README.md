...
| `;저장`, `;뒤로` | **UI 요소 검색** — 최전면 앱의 버튼·링크·메뉴를 이름으로 찾아 Enter로 클릭 (`;`만 치면 전체 목록, 손쉬운 사용 권한 필요) |
...
힌트 모드에서 **⌃Space 한 번 더** (또는 **Tab**) → 스크롤 모드. 포인터가 최전면 창 중앙으로 이동하고:

| 키 | 동작 |
|---|---|
| H / J / K / L | 좌 / 아래 / 위 / 우 스크롤 (꾹 누르면 연속) |
| D / U | 반 페이지 아래 / 위 |
| Esc 또는 ⌃Space | 종료 |

### 그리드 모드 (접근성 트리 없는 앱 폴백)

힌트 모드에서 **`/`** → 그리드 모드. 요소를 하나도 못 찾는 앱(일부 게임·커스텀 렌더링 앱)에서는 자동으로 그리드로 폴백된다.

| 키 | 동작 |
|---|---|
| Q W E / A S D / Z X C | 화면을 3×3로 분할, 자판 배치 그대로 칸 선택 — 누를 때마다 재귀 분할로 좁혀짐 |
| Return 또는 Space | 현재 영역 중앙 클릭 |
| Esc 또는 ⌃Space | 종료 |

3~4번 누르면 2560px 화면에서 ~30px 정밀도에 도달한다.
