...
- **규칙 목록**: 앱 아이콘·이름 + 소스 드롭다운(변경) + ✕(삭제)
- **앱 추가**: 실행 중인 앱 드롭다운 + "기타 앱 선택…"(NSOpenPanel). 추가 시 현재 입력 소스로 등록
- **인디케이터 옵션**: 표시 켜기/끄기, 표시 시간(0.5~3초)
- 런처 `입력규칙` 명령과 공존 — 빠른 등록은 런처, 조망·일괄 관리는 설정 창

저장 형식은 `InputSourceSettings { rules, indicatorEnabled, indicatorDuration }`로 승격.
구식 `[bundleID: sourceID]` 딕셔너리 파일은 로드 시 자동 마이그레이션
(구식 포맷은 `[String: String]` 디코드가 성공하는 것으로 판별 — 신형은 rules 값이 객체라 실패).

## 로드맵

- [x] 1단계: 자동 전환 + 인디케이터 + 런처 명령 (2026-07-20)
- [x] 2단계: 설정 창 "입력 소스" 탭 — 규칙 관리 UI, 인디케이터 옵션 (2026-07-20)
